...
bool ImGui::ButtonEx( const char* label , const ImVec2& size_arg , bool underline , ImGuiButtonFlags flags )
{
	ImGuiWindow* window = GetCurrentWindow( );
	if ( window->SkipItems )
		return false;
	...
	// Render
	const auto col = GetColorU32( ( held && hovered ) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button );
	RenderNavHighlight( bb , id );
	RenderFrame( bb.Min , bb.Max , col , true , style.FrameRounding );
	RenderTextClipped( bb.Min + style.FramePadding , bb.Max - style.FramePadding , label , NULL , &label_size , style.ButtonTextAlign , &bb );

	if ( underline )
		window->DrawList->AddLine( bb.Min + ImVec2( 0 , size_arg.y ) , bb.Min + ImVec2( bb.Max.x , size_arg.y ) , ImColor( 140 , 74 , 207 , 255 ) , 2 );

	IMGUI_TEST_ENGINE_ITEM_INFO( id , label , window->DC.LastItemStatusFlags );
	return pressed;
}

bool ImGui::Button( const char* label , bool underline , const ImVec2& size_arg )
{
	return ButtonEx( label , size_arg , underline , ImGuiButtonFlags_None );
}

bool ImGui::ButtonEx2( const char* first_label , const char* second_label , ImFont* font , const ImVec2& size_arg , ImGuiButtonFlags flags )
{
	ImGuiWindow* window = GetCurrentWindow( );
	if ( window->SkipItems )
		return false;
	...
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	const ImGuiID id = window->GetID( first_label );
	const ImVec2 label_size = CalcTextSize( first_label , NULL , true );
	PushFont( font );
	const ImVec2 label_size2 = CalcTextSize( second_label , NULL , true );
	PopFont( );
	...
	RenderTextClipped( bb.Min + style.FramePadding , bb.Max - style.FramePadding , first_label , NULL , &label_size , style.ButtonTextAlign - ImVec2( 0.3f , 0.1f ) , &bb );

	PushFont( font );
	RenderTextClipped( bb.Min + style.FramePadding , bb.Max - style.FramePadding , second_label , NULL , &label_size2 , style.ButtonTextAlign , &bb );
	PopFont( );
...
bool ImGui::Button2( const char* first_label , const char* second_label , ImFont* font , const ImVec2& size_arg )
{
	return ButtonEx2( first_label , second_label , font , size_arg , 0 );
}
...
bool ImGui::SmallButton( const char* label )
{
	...
	bool pressed = ButtonEx( label , ImVec2( 0 , 0 ) , false , ImGuiButtonFlags_AlignTextBaseLine );
...
bool ImGui::Keybind( const char* str_id , int* current_key , int* key_style ) {
	...
	const ImRect frame_bb( window->DC.CursorPos , window->DC.CursorPos + ImVec2( window->Size.x - window->WindowPadding.x * 2 , label_size.y ) );
	const ImRect total_bb( window->DC.CursorPos + ImVec2( str_id_size.x / 2 , 0 ) , window->DC.CursorPos + ImVec2( frame_bb.Max.x , label_size.y ) );
	...
	if ( g.ActiveId == id ) {
		...
		if ( key_style ) {
			...
			if ( popup_open ) {
				SetNextWindowSize( ImVec2( 100 , 82 ) );
				...
				if ( Selectable( "force off" , *key_style == 3 ) )
					*key_style = 3;
	...
	const auto buf_size = CalcTextSize( buf_display , NULL , false );
	ImGui::RenderText( ImVec2( frame_bb.Max.x - buf_size.x , frame_bb.Min.y ) , buf_display );
...
bool ImGui::Checkbox( const char* label , bool* v )
{
	...
	if ( window->SkipItems )
		return false;
	if ( !g_anim.infos.count( label ) ) {
		g_anim.infos[ label ] = new Anim( );
	}
	...
	//const ImRect occupied_bb( pos , window->DC.CursorPos + ImVec2( label_size.x > 0.0f ? square_sz + style.ItemInnerSpacing.x + label_size.x : 0.0f , 12.f ) );
	...
	ImColor check_col = ImColor( 0.50f , 0.30f , 0.78f , 1.f );
	const ImRect _pos( pos , pos + ImVec2( square_sz / 2 , square_sz / 2 ) );

	if ( *v )
		g_anim.infos[ label ]->checkbox_anim += 16;
	else
		g_anim.infos[ label ]->checkbox_anim -= 16;

	g_anim.infos[ label ]->checkbox_anim = std::clamp<float>( g_anim.infos[ label ]->checkbox_anim , 0 , 255 );

	if ( g_anim.infos[ label ]->checkbox_anim > 62 )
		window->DrawList->AddRectFilled( _pos.Min + ImVec2( 0 , square_sz / 2 ) , _pos.Max + ImVec2( 0 , square_sz / 2 ) , check_col );//bottom left
	if ( g_anim.infos[ label ]->checkbox_anim > 124 )
		window->DrawList->AddRectFilled( _pos.Min , _pos.Max , check_col );//top left
	if ( g_anim.infos[ label ]->checkbox_anim > 186 )
		window->DrawList->AddRectFilled( _pos.Min + ImVec2( square_sz / 2 , 0 ) , _pos.Max + ImVec2( square_sz / 2 , 0 ) , check_col ); //top right
	if ( g_anim.infos[ label ]->checkbox_anim > 250 )
		window->DrawList->AddRectFilled( _pos.Min + ImVec2( square_sz / 2 , square_sz / 2 ) , _pos.Max + ImVec2( square_sz / 2 , square_sz / 2 ) , check_col ); //bottom right


	if ( g.LogEnabled )
		LogRenderedText( &total_bb.Min , *v ? "[x]" : "[ ]" );
	if ( label_size.x > 0.0f )
		RenderText( ImVec2( check_bb.Max.x + style.ItemInnerSpacing.x , check_bb.Min.y + style.FramePadding.y - 1 ) , label );
...
bool ImGui::BeginCombo( const char* label , const char* preview_value , ImGuiComboFlags flags )
{
	...
	const float expected_w = window->Size.x - window->WindowPadding.x * 2;
	const float w = ( flags & ImGuiComboFlags_NoPreview ) ? arrow_size : expected_w;

	const ImRect frame_bb( window->DC.CursorPos , window->DC.CursorPos + ImVec2( w , 17.f/*label_size.y + style.FramePadding.y * 2.0f*/ ) );
	...
	//	window->DrawList->AddRectFilledMultiColor( frame_bb.Min , ImVec2( value_x2 , frame_bb.Max.y ) , frame_col , frame_col , ImColor( 0.25f , 0.25f , 0.25f , 1.f ) , ImColor( 0.25f , 0.25f , 0.25f , 1.f ) );
	...
	if ( preview_value != NULL && !( flags & ImGuiComboFlags_NoPreview ) )
		RenderTextClipped( ( frame_bb.Min + ImVec2( ( expected_w / 2 ) - preview_size.x / 2 , 2 ) ) , ImVec2( value_x2 , frame_bb.Max.y ) , preview_value , NULL , NULL , ImVec2( 0.0f , 0.0f ) );

	if ( label_size.x > 0 )
		RenderText( ImVec2( frame_bb.Min.x , frame_bb.Min.y - label_size.y * 1.25f ) , label );


	//if ( preview_value != NULL && !( flags & ImGuiComboFlags_NoPreview ) )
	//	RenderTextClipped( frame_bb.Min + ImVec2( w / 2 - preview_size.x / 2 , 17.f / 2 - preview_size.y / 2 )/*+ style.FramePadding*/ , ImVec2( value_x2 , frame_bb.Max.y ) , preview_value , NULL , NULL , ImVec2( 0.0f , 0.0f ) );

	//if ( label_size.x > 0 )
	//	RenderText( ImVec2( frame_bb.Max.x + style.ItemInnerSpacing.x , frame_bb.Min.y /*+ style.FramePadding.y*/ + 17 / 2.f - label_size.y / 2 ) , label );
	...
	// Horizontally align ourselves with the framed text
	PushStyleVar( ImGuiStyleVar_WindowPadding , ImVec2( style.FramePadding.x , style.WindowPadding.y - 5 ) );
	bool ret = Begin( name , NULL , window_flags );
	PopStyleVar( );
	if ( !ret )
	{
...
void ImGui::Slider( const char* name , int* val , int min , int max , bool aimbot )
{
	// CHANGE SPACING SLIDER
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	const auto name_size = CalcTextSize( name , NULL , true );
	ImGui::SetCursorPosY( ImGui::GetCursorPosY( ) + style.FramePadding.y * 2 + name_size.y );
	...
	const ImRect frame( window->DC.CursorPos , window->DC.CursorPos + ImVec2( window->Size.x - window->WindowPadding.x * 2 , 12 ) );
	const ImRect total_bb( frame.Min , window->DC.CursorPos + ImVec2( name_size.y > 0.0f ? style.ItemInnerSpacing.y + name_size.y : 0.0f , 12.f ) );

	ItemSize( total_bb , -1 );
	if ( !ItemAdd( total_bb , id , &frame ) )
		return;

	RenderNavHighlight( frame , id );
	RenderFrame( frame.Min , frame.Max , ImColor( 0.2f , 0.2f , 0.2f , 1.f ) , true , g.Style.FrameRounding );
	const int max_size = window->Size.x - window->WindowPadding.x * 2;
	const auto hovered = ItemHoverable( ImRect( frame.Min , frame.Min + ImVec2( window->Size.x + 5 , 12 ) ) , id );
	if ( hovered && g.IO.MouseDown[ 0 ] && g.ActiveId != id )
		g.ActiveId = id;

	if ( g.ActiveId == id && g.IO.MouseDown[ 0 ] )
	{
		float value_unmapped = std::clamp( static_cast< float >( g.IO.MousePos.x - window->DC.CursorPos.x ) , 0.0f , static_cast< float >( max_size ) );
		float value_mapped = static_cast< float >( value_unmapped / max_size * ( max - min ) + min );
		*val = value_mapped;
	...
	if ( g.ActiveIdSource == ImGuiInputSource_Mouse && g.ActiveId == id && !hovered )
	{
		if ( !g.IO.MouseDown[ 0 ] )
			ClearActiveID( );
	}
	...
	const float dynamic_width = ( static_cast< float >( *val ) - min ) / ( max - min ) * max_size;

	char buffer[ 512 ];
	sprintf_s( buffer , aimbot && *val > 100 ? "hp + %d" : "%d" , aimbot && *val > 100 ? *val - 100 : *val );
	const auto val_size = CalcTextSize( buffer , NULL , false );

	RenderFrame( frame.Min , frame.Min + ImVec2( dynamic_width , 12 ) , ImColor( 0.50f , 0.30f , 0.78f , 1.f ) , true , g.Style.FrameRounding );

	RenderText( ImVec2( frame.Min.x + name_size.x + 4 , frame.Min.y - name_size.y * 1.25f ) , buffer );

	if ( name_size.x > 0.0f )
		RenderText( ImVec2( frame.Min.x , frame.Min.y - name_size.y * 1.25f ) , name );

	if ( !g.IO.MouseDown[ 0 ] && hovered )
	{
		float value_unmapped2 = std::clamp( static_cast< float >( g.IO.MousePos.x - window->DC.CursorPos.x ) , 0.0f , static_cast< float >( max_size ) );
		float value_mapped2 = static_cast< float >( value_unmapped2 / max_size * ( max - min ) + min );
		int preview_value2 = value_mapped2;

		const float dynamic_width2 = ( static_cast< float >( preview_value2 ) - min ) / ( max - min ) * max_size;

		if ( preview_value2 > max )
			preview_value2 = max;
		if ( preview_value2 < min )
			preview_value2 = min;

		char buffer2[ 512 ];

		sprintf_s( buffer2 , aimbot && preview_value2 > 100 ? "[hp + %d]" : "[%d]" , aimbot && preview_value2 > 100 ? preview_value2 - 100 : preview_value2 );

		const auto val_size2 = CalcTextSize( buffer2 , NULL , false );

		RenderFrame( frame.Min , frame.Min + ImVec2( dynamic_width2 , 12 ) , ImColor( 0.50f , 0.30f , 0.78f , 0.3f ) , true , g.Style.FrameRounding );

		const auto o_col = g.Style.Colors[ ImGuiCol_Text ];
		g.Style.Colors[ ImGuiCol_Text ] = ImVec4( o_col.x , o_col.y , o_col.z , 0.3f );
		RenderText( ImVec2( frame.Min.x + name_size.x + 6 + val_size.x , frame.Min.y - name_size.y * 1.25f ) , buffer2 );
		g.Style.Colors[ ImGuiCol_Text ] = o_col;
	}
}
...
bool ImGui::SliderScalar( const char* label , ImGuiDataType data_type , void* p_data , const void* p_min , const void* p_max , const char* format , ImGuiSliderFlags flags )
{
	...
	const ImVec2 label_size = CalcTextSize( label , NULL , true );
	const ImRect frame_bb( window->DC.CursorPos , window->DC.CursorPos + ImVec2( w , 10/*label_size.y + style.FramePadding.y * 2.0f*/ ) );
	const ImRect total_bb( frame_bb.Min , frame_bb.Max + ImVec2( label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f , 12.f ) );
	...
	const bool hovered = ItemHoverable( frame_bb , id );
	const bool temp_input_allowed = /*( flags & ImGuiSliderFlags_NoInput ) == 0*/ false;
	bool temp_input_is_active = temp_input_allowed && TempInputIsActive( id );
	if ( !temp_input_is_active )
	{
		const bool focus_requested = temp_input_allowed && FocusableItemRegister( window , id );
		...
		if ( focus_requested || clicked || g.NavActivateId == id || g.NavInputId == id )
		{
			...
			{
				temp_input_is_active = false; // true
				FocusableItemUnregister( window );
			}
		}
	}
	...
	const ImU32 frame_col = GetColorU32( g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg );
	RenderNavHighlight( frame_bb , id );
	RenderFrame( frame_bb.Min , frame_bb.Max - ImVec2( 2.f , 2.f ) , frame_col , true , g.Style.FrameRounding );
	...
	if ( value_changed )
		MarkItemEdited( id );
	grab_bb.Min += ImVec2( 2.f , 0.f );
	grab_bb.Max += ImVec2( 2.f , 0.f );
	// Render grab
	...
	char value_buf[ 64 ];
	const char* value_buf_end = value_buf + DataTypeFormatString( value_buf , IM_ARRAYSIZE( value_buf ) , data_type , p_data , format );
	RenderTextClipped( frame_bb.Min + ImVec2( 0.f , 4.f ) , frame_bb.Max + ImVec2( 0.f , 12.f ) , value_buf , value_buf_end , NULL , ImVec2( 0.5f , 0.5f ) );

	if ( label_size.x > 0.0f )
		RenderText( ImVec2( frame_bb.Min.x + style.ItemInnerSpacing.x , frame_bb.Min.y - label_size.y * 1.55f ) , label );
...
bool ImGui::InputScalar( const char* label , ImGuiDataType data_type , void* p_data , const void* p_step , const void* p_step_fast , const char* format , ImGuiInputTextFlags flags )
{
	...
	if ( p_step != NULL )
	{
		...
		if ( ButtonEx( "-" , ImVec2( button_size , button_size ) , false , button_flags ) )
		{
...
bool ImGui::ColorEdit4( const char* label , float col[ 4 ] , ImGuiColorEditFlags flags )
{
	...
	{
		window->DC.CursorPos = ImVec2( pos.x + CalcTextSize( label , NULL , true ).x + w_button , pos.y );
	...
	if ( label != label_display_end && !( flags & ImGuiColorEditFlags_NoLabel ) )
	{
		...
		window->DC.CursorPos = ImVec2( pos.x , pos.y - 8 + CalcTextSize( label , NULL , true ).y / 2 );
		TextEx( label , label_display_end );
	}
...
bool ImGui::ColorButton( const char* desc_id , const ImVec4& col , ImGuiColorEditFlags flags , ImVec2 size )
{
	...
	window->DC.CursorPos.y += 2.f;
	if ( size.x == 0.0f )
		size.x = 20;
	if ( size.y == 0.0f )
		size.y = 10;
	const ImRect bb( window->DC.CursorPos , window->DC.CursorPos + size );
	ItemSize( bb , ( size.y >= 10 ) ? g.Style.FramePadding.y : 0.0f );
	if ( !ItemAdd( bb , id ) )
		return false;
...
bool ImGui::Selectable( const char* label , bool selected , ImGuiSelectableFlags flags , const ImVec2& size_arg )
{
	...
	RenderTextClipped( bb.Min + ImVec2( size.x / 2 - label_size.x / 2 + style.ItemSpacing.x / 2 + 1 , 2 ) , bb.Max , label , NULL , &label_size , style.SelectableTextAlign , &bb );
...
//-------------------------------------------------------------------------
float ImGui::AnimTime( )
{
	return interfaces::global_vars->frametime * 10;
}
