...
struct ImBitvec3_t;                 // Store 1-bit per value
...
// Enforce cdecl calling convention for functions called by the standard library, in case compilation settings changed the default to e.g. __vec3_tcall
#ifdef _MSC_VER
...
// Helper: ImVec2ih (2D vec3_t, half-size integer, for long-term packed storage)
struct ImVec2ih
{
	short   x , y;
	ImVec2ih( ) { x = y = 0; }
...
struct IMGUI_API ImBitvec3_t
{
	Imvec3_t<ImU32> Storage;
...
{
	Imvec3_t<T>     Buf;        // Contiguous data
...
{
	Imvec3_t<ImDrawList*>   Layers[ 2 ];           // Global layers for: regular, tooltip
...
struct IMGUI_API ImGuiInputTextState
{
	...
	Imvec3_t<ImWchar>       TextW;                  // edit buffer, we need to persist but can't guarantee the persistence of the user-provided buffer. so we copy into own buffer.
	Imvec3_t<char>          TextA;                  // temporary UTF8 buffer for callbacks and other operations. this is not updated in every code-path! size=capacity.
	Imvec3_t<char>          InitialTextA;           // backup of end-user buffer at the time of focus (in UTF-8, unaltered)
...
struct ImGuiColumns
{
	...
	Imvec3_t<ImGuiColumnData> Columns;
...
struct ImGuiContext
{
	...
	// Windows state
	Imvec3_t<ImGuiWindow*>  Windows;                            // Windows, sorted in display order, back to front
	Imvec3_t<ImGuiWindow*>  WindowsFocusOrder;                  // Windows, sorted in focus order, back to front. (FIXME: We could only store root windows here! Need to sort out the Docking equivalent which is RootWindowDockStop and is unfortunately a little more dynamic)
	Imvec3_t<ImGuiWindow*>  WindowsTempSortBuffer;              // Temporary buffer used in EndFrame() to reorder windows so parents are kept before their child
	Imvec3_t<ImGuiWindow*>  CurrentWindowStack;
	...
	// Shared stacks
	Imvec3_t<ImGuiColorMod> ColorModifiers;                     // Stack for PushStyleColor()/PopStyleColor()
	Imvec3_t<ImGuiStyleMod> StyleModifiers;                     // Stack for PushStyleVar()/PopStyleVar()
	Imvec3_t<ImFont*>       FontStack;                          // Stack for PushFont()/PopFont()
	Imvec3_t<ImGuiPopupData>OpenPopupStack;                     // Which popups are open (persistent)
	Imvec3_t<ImGuiPopupData>BeginPopupStack;                    // Which level of BeginPopup() we are in (reset every frame)
	...
	Imvec3_t<unsigned char> DragDropPayloadBufHeap;             // We don't expose the Imvec3_t<> directly, ImGuiPayload only holds pointer+size
	unsigned char           DragDropPayloadBufLocal[ 16 ];        // Local buffer for small payloads

	// Tab bars
	ImGuiTabBar* CurrentTabBar;
	ImPool<ImGuiTabBar>             TabBars;
	Imvec3_t<ImGuiPtrOrIndex>       CurrentTabBarStack;
	Imvec3_t<ImGuiShrinkWidthItem>  ShrinkWidthBuffer;
	...
	Imvec3_t<char>          ClipboardHandlerData;               // If no custom clipboard handler is defined
	Imvec3_t<ImGuiID>       MenusIdSubmittedThisFrame;          // A list of menu IDs that were rendered at least once
	...
	Imvec3_t<ImGuiSettingsHandler>      SettingsHandlers;       // List of .ini settings handlers
	ImChunkStream<ImGuiWindowSettings>  SettingsWindows;        // ImGuiWindow .ini settings entries
	Imvec3_t<ImGuiContextHook>          Hooks;                  // Hooks for extensions (e.g. test engine)
...
struct IMGUI_API ImGuiWindowTempData
{
	...
	Imvec3_t<ImGuiWindow*>  ChildWindows;
	...
	// We store the current settings outside of the vec3_ts to increase memory locality (reduce cache misses). The vec3_ts are rarely modified. Also it allows us to not heap allocate for short-lived windows which are not using those settings.
	ImGuiItemFlags          ItemFlags;              // == ItemFlagsStack.back() [empty == ImGuiItemFlags_Default]
	float                   ItemWidth;              // == ItemWidthStack.back(). 0.0: default, >0.0: width in pixels, <0.0: align xx pixels to the right of window
	float                   TextWrapPos;            // == TextWrapPosStack.back() [empty == -1.0f]
	Imvec3_t<ImGuiItemFlags>ItemFlagsStack;
	Imvec3_t<float>         ItemWidthStack;
	Imvec3_t<float>         TextWrapPosStack;
	Imvec3_t<ImGuiGroupData>GroupStack;
...
struct IMGUI_API ImGuiWindow
{
	...
	Imvec3_t<ImGuiID>       IDStack;                            // ID stack. ID are hashes seeded with the value at the top of the stack. (In theory this should be in the TempData structure)
	...
	Imvec3_t<ImGuiColumns>  ColumnsStorage;
...
namespace ImGui
{
	...
	IMGUI_API bool			AnimChildEx( const char* name , ImGuiID id , const ImVec2& size_arg , bool border , ImGuiWindowFlags flags );
	IMGUI_API bool          BeginChildEx( const char* name , ImGuiID id , const ImVec2& size_arg , bool border , ImGuiWindowFlags flags,bool pad = false );
...
struct Anim
{
	Anim( ) = default;
	float checkbox_anim;
	inline static ImVec2 child_wishsize;
	inline static bool child_reached_size;
	inline static float alpha;
	inline static float alpha2 = 1.f;
	inline static bool child_open;
};
...
public:
	ImAnim( ) = default;
	std::unordered_map< const char* , Anim* > infos;
};

extern ImAnim g_anim;
#if defined(__clang__)
