...
/*
 ...
 - C++: pay attention that Imvec3_t<> manipulates plain-old-data and does not honor construction/destruction (avoid using it in your code!).
...
static void             AddDrawListToDrawData( Imvec3_t<ImDrawList*>* out_list , ImDrawList* draw_list );
static void             AddWindowToSortBuffer( Imvec3_t<ImGuiWindow*>* out_sorted_windows , ImGuiWindow* window );
...
// Otherwise, you probably don't want to modify them mid-program, and if you use global/static e.g. Imvec3_t<> instances you may need to keep them accessible during program destruction.
...
static ImGuiStorage::ImGuiStoragePair* LowerBound( Imvec3_t<ImGuiStorage::ImGuiStoragePair>& data , ImGuiID key )
{
...
{
	ImGuiStoragePair* it = LowerBound( const_cast< Imvec3_t<ImGuiStoragePair>& >( Data ) , key );
...
void ImGuiTextFilter::ImGuiTextRange::split( char separator , Imvec3_t<ImGuiTextRange>* out ) const
{
	out->resize( 0 );
	const char* wb = b;
	const char* we = wb;
...
static void AddWindowToDrawData( Imvec3_t<ImDrawList*>* out_render_list , ImGuiWindow* window )
{
	ImGuiContext& g = *GImGui;
...
void ImDrawDataBuilder::FlattenIntoSingleLayer( )
{
	...
	{
		Imvec3_t<ImDrawList*>& layer = Layers[ layer_n ];
		if ( layer.empty( ) )
			continue;
...
static void SetupDrawData( Imvec3_t<ImDrawList*>* draw_lists , ImDrawData* draw_data )
{
	ImGuiIO& io = ImGui::GetIO( );
...
bool ImGui::BeginChildEx( const char* name , ImGuiID id , const ImVec2& size_arg , bool border , ImGuiWindowFlags flags , bool pad )
{
	ImGuiContext& g = *GImGui;
...
bool ImGui::BeginChild( const char* str_id , const ImVec2& size_arg , bool border , ImGuiWindowFlags extra_flags , bool pad )
{
	ImGuiWindow* window = GetCurrentWindow( );
	return BeginChildEx( str_id , window->GetID( str_id ) , size_arg , border , extra_flags , pad );
}

bool ImGui::BeginChild( ImGuiID id , const ImVec2& size_arg , bool border , ImGuiWindowFlags extra_flags , bool pad )
{
	IM_ASSERT( id != 0 );
	return BeginChildEx( NULL , id , size_arg , border , extra_flags , pad );
}

void ImGui::EndChild( bool title )
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	if ( title )
	{
		ImGui::PushFont( g.IO.CustomFont );
		...
		ImColor upleft = ImColor( 78 , 40 , 130 , 255 );
		ImColor upright = ImColor( 126 , 96 , 166 , 255 );
		ImColor downleft = ImColor( 126 , 96 , 166 , 255 );
		ImColor downright = ImColor( 78 , 40 , 130 , 255 );

		ImVec2 text_position( window->Pos.x + 4 , window->Pos.y - 6 + CalcTextSize( window_title.c_str( ) ).y / 2 );
		ImRect rect_pos( window->Pos , window->Pos + ImVec2( window->Size.x , 12 ) );
		PushClipRect( rect_pos.Min , rect_pos.Max , false );
		ImGui::GetWindowDrawList( )->AddRectFilledMultiColor( rect_pos.Min , rect_pos.Max , upleft , upright , downleft , downright );
		ImGui::GetWindowDrawList( )->AddText( ImVec2( text_position.x - 1 , text_position.y - 1 ) , IM_COL32( 0 , 0 , 0 , 30 ) , window_title.c_str( ) );
		ImGui::GetWindowDrawList( )->AddText( ImVec2( text_position.x - 1 , text_position.y ) , IM_COL32( 0 , 0 , 0 , 30 ) , window_title.c_str( ) );
		ImGui::GetWindowDrawList( )->AddText( ImVec2( text_position.x - 1 , text_position.y + 1 ) , IM_COL32( 0 , 0 , 0 , 30 ) , window_title.c_str( ) );
		ImGui::GetWindowDrawList( )->AddText( ImVec2( text_position.x , text_position.y - 1 ) , IM_COL32( 0 , 0 , 0 , 30 ) , window_title.c_str( ) );
		ImGui::GetWindowDrawList( )->AddText( ImVec2( text_position.x , text_position.y + 1 ) , IM_COL32( 0 , 0 , 0 , 30 ) , window_title.c_str( ) );
		ImGui::GetWindowDrawList( )->AddText( ImVec2( text_position.x + 1 , text_position.y - 1 ) , IM_COL32( 0 , 0 , 0 , 30 ) , window_title.c_str( ) );
		ImGui::GetWindowDrawList( )->AddText( ImVec2( text_position.x + 1 , text_position.y ) , IM_COL32( 0 , 0 , 0 , 30 ) , window_title.c_str( ) );
		ImGui::GetWindowDrawList( )->AddText( ImVec2( text_position.x + 1 , text_position.y + 1 ) , IM_COL32( 0 , 0 , 0 , 30 ) , window_title.c_str( ) );
	...
	/*clip_rect.Min.x = ImFloor( 0.5f + title_bar_rect.Min.x + ImMax( window->WindowBorderSize , ImFloor( window->WindowPadding.x * 0.1f ) ) );
	clip_rect.Min.y = ImFloor( 0.5f + title_bar_rect.Max.y + window->MenuBarHeight( ) + window->WindowBorderSize ) - 16;
	clip_rect.Max.x = ImFloor( 0.5f + window->Pos.x + window->Size.x - window->ScrollbarSizes.x - ImMax( window->WindowBorderSize , ImFloor( window->WindowPadding.x * 0.1f ) ) );
	clip_rect.Max.y = ImFloor( 0.5f + window->Pos.y + window->Size.y - window->ScrollbarSizes.y - window->WindowBorderSize );*/

	//PushClipRect( clip_rect.Min , clip_rect.Max , false );

	//ImGui::GetWindowDrawList( )->AddLine( line_min , line_max , GetColorU32( ImGuiCol_ChildBg ) );
...
bool ImGui::AnimChildEx( const char* name , ImGuiID id , const ImVec2& size_arg , bool border , ImGuiWindowFlags flags )
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* parent_window = g.CurrentWindow;

	if ( !g_anim.infos.count( name ) ) {
		g_anim.infos[ name ] = new Anim( );
	}

	flags |= ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_UsePadding;
	flags |= ( parent_window->Flags & ImGuiWindowFlags_NoMove );  // Inherit the NoMove flag
	...
	g_anim.infos[ name ]->child_wishsize.y = size.y;

	if ( g_anim.infos[ name ]->child_open )
	{
		g_anim.infos[ name ]->alpha2 -= AnimTime( ) / 2.f;

		if ( g_anim.infos[ name ]->alpha2 < 0.3f && g_anim.infos[ name ]->child_reached_size )
			g_anim.infos[ name ]->alpha += AnimTime( ) / 2.f;
	}
	else
	{
		if ( g_anim.infos[ name ]->child_wishsize.x == 70 )
			g_anim.infos[ name ]->alpha2 += AnimTime( ) / 2.f;
		g_anim.infos[ name ]->alpha -= AnimTime( ) / 2.f;
	}
	g_anim.infos[ name ]->alpha = std::clamp<float>( g_anim.infos[ name ]->alpha , -0.4f , 1.f );
	g_anim.infos[ name ]->alpha2 = std::clamp<float>( g_anim.infos[ name ]->alpha2 , -0.5f , 1.f );

	if ( g_anim.infos[ name ]->child_open/* == 1.f*/ )
		g_anim.infos[ name ]->child_wishsize.x += ( ( AnimTime( ) / 10 ) * size.x );
	else if ( g_anim.infos[ name ]->alpha <= 0.4f )
		g_anim.infos[ name ]->child_wishsize.x -= ( ( AnimTime( ) / 10 ) * size.x );


	g_anim.infos[ name ]->child_wishsize.x = std::clamp<float>( g_anim.infos[ name ]->child_wishsize.x , 70 , size.x );
	g_anim.infos[ name ]->child_reached_size = g_anim.infos[ name ]->child_wishsize.x >= size.x - 1;

	SetNextWindowSize( g_anim.infos[ name ]->child_wishsize );
	...
	bool ret = Begin( title , NULL , flags );
	ImGui::PushStyleColor( ImGuiCol_Button , g.Style.Colors[ ImGuiCol_ChildBg ] );
	ImGui::PushStyleColor( ImGuiCol_ButtonActive , g.Style.Colors[ ImGuiCol_ChildBg ] );
	ImGui::PushStyleColor( ImGuiCol_ButtonHovered , g.Style.Colors[ ImGuiCol_ChildBg ] );

	ImGui::PushFont( g.IO.ButtonFont );
	if ( ImGui::Button( "moonlight" , true , ImVec2( -1 , 40 ) ) )
		g_anim.infos[ name ]->child_open = !g_anim.infos[ name ]->child_open;
	ImGui::PopFont( );
...
bool ImGui::BeginAnimChild( const char* str_id , const ImVec2& size_arg , bool border , ImGuiWindowFlags extra_flags )
{
	ImGuiWindow* window = GetCurrentWindow( );
	return AnimChildEx( str_id , window->GetID( str_id ) , size_arg , border , extra_flags );
}

bool ImGui::BeginAnimChild( ImGuiID id , const ImVec2& size_arg , bool border , ImGuiWindowFlags extra_flags )
{
	IM_ASSERT( id != 0 );
	return AnimChildEx( NULL , id , size_arg , border , extra_flags );
}
...
bool ImGui::Begin( const char* name , bool* p_open , ImGuiWindowFlags flags )
{
	...
	if ( first_begin_of_the_frame )
	{
		...
		//if ( g_anim.infos[ name ]->combo_reached_size )
		//{
...
static ImVec2 CalcNextScrollFromScrollTargetAndClamp( ImGuiWindow* window )
{
	...
	if ( window->ScrollTarget.x < FLT_MAX )
	{
		...
		scroll.x = scroll_target_x - center_x_ratio * ( window->SizeFull.x );
	}
	if ( window->ScrollTarget.y < FLT_MAX )
	{
		...
		scroll.y = scroll_target_y - center_y_ratio * ( window->SizeFull.y - decoration_up_height );
	}
...
void ImGui::LogButtons( )
{
	...
	PushID( "LogButtons" );
#ifndef IMGUI_DISABLE_TTY_FUNCTIONS
	const bool log_to_tty = Button( "Log To TTY" , false ); SameLine( );
#else
	const bool log_to_tty = false;
#endif
	const bool log_to_file = Button( "Log To File" , false ); SameLine( );
	const bool log_to_clipboard = Button( "Log To Clipboard" , false ); SameLine( );
...
void ImGui::ShowMetricsWindow( bool* p_open )
{
	...
	struct Funcs
	{
		...
		static void NodeWindows( Imvec3_t<ImGuiWindow*>& windows , const char* label )
		{
	...
	if ( ImGui::TreeNode( "Tools" ) )
	{
		...
		if ( ImGui::Button( "Item Picker.." , false ) )
