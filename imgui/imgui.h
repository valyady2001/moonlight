...
// 2D vec3_t (often used to store positions or sizes)
struct ImVec2
{
	float                                   x , y;
	ImVec2( ) { x = y = 0.0f; }
...
// 4D vec3_t (often used to store floating-point colors)
struct ImVec4
{
	float                                   x , y , z , w;
	ImVec4( ) { x = y = z = w = 0.0f; }
...
namespace ImGui
{
	...
	IMGUI_API bool          BeginChild( const char* str_id , const ImVec2& size = ImVec2( 0 , 0 ) , bool border = false , ImGuiWindowFlags flags = 0 , bool pad = false );
	IMGUI_API bool          BeginAnimChild( const char* str_id , const ImVec2& size = ImVec2( 0 , 0 ) , bool border = false , ImGuiWindowFlags flags = 0 );
	IMGUI_API bool          BeginAnimChild( ImGuiID id , const ImVec2& size = ImVec2( 0 , 0 ) , bool border = false , ImGuiWindowFlags flags = 0 );
	IMGUI_API bool          BeginChild( ImGuiID id , const ImVec2& size = ImVec2( 0 , 0 ) , bool border = false , ImGuiWindowFlags flags = 0 , bool pad = false );
	IMGUI_API void          EndChild( bool title = false );
	...
	IMGUI_API bool ButtonEx( const char* label , const ImVec2& size_arg , bool underline , ImGuiButtonFlags flags );
	IMGUI_API bool Button( const char* label , bool underline , const ImVec2& size = ImVec2( 0 , 0 ) );

	IMGUI_API bool ButtonEx2( const char* first_label , const char* second_label , ImFont* font , const ImVec2& size_arg , ImGuiButtonFlags flags );
	IMGUI_API bool Button2( const char* first_label , const char* second_label , ImFont* font , const ImVec2& size_arg );
	...
	IMGUI_API void			Slider( const char* name , int* val , int min , int max , bool aimbot = false );

	// Widgets: Drag Sliders
	// - CTRL+Click on any drag box to turn them into an input box. Manually input values aren't clamped and can go off-bounds.
	// - For all the Float2/Float3/Float4/Int2/Int3/Int4 versions of every functions, note that a 'float v[X]' function argument is the same as 'float* v', the array syntax is just a way to document the number of elements that are expected to be accessible. You can pass address of your first element out of a contiguous set, e.g. &myvec3_t.x
	...
	// - You can pass the address of a first float element out of a contiguous structure, e.g. &myvec3_t.x
...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_UsePadding = 1 << 29   // Don't use! For internal use by BeginMenu()
...
enum ImGuiCol_
{
	...
	ImGuiCol_ButtonAnimActive ,
	ImGuiCol_ButtonAnimInactive ,
...
struct Imvec3_t
{
	...
	inline Imvec3_t( ) { Size = Capacity = 0; Data = NULL; }
	inline Imvec3_t( const Imvec3_t<T>& src ) { Size = Capacity = 0; Data = NULL; operator=( src ); }
	inline Imvec3_t<T>& operator=( const Imvec3_t<T>& src ) { clear( ); resize( src.Size ); memcpy( Data , src.Data , ( size_t ) Size * sizeof( T ) ); return *this; }
	inline ~Imvec3_t( ) { if ( Data ) IM_FREE( Data ); }
	...
	inline void         swap( Imvec3_t<T>& rhs ) { int rhs_size = rhs.Size; rhs.Size = Size; Size = rhs_size; int rhs_cap = rhs.Capacity; rhs.Capacity = Capacity; Capacity = rhs_cap; T* rhs_data = rhs.Data; rhs.Data = Data; Data = rhs_data; }

	inline int          _grow_capacity( int sz ) const { int new_capacity = Capacity ? ( Capacity + Capacity / 2 ) : 8; return new_capacity > sz ? new_capacity : sz; }
	inline void         resize( int new_size ) { if ( new_size > Capacity ) reserve( _grow_capacity( new_size ) ); Size = new_size; }
	inline void         resize( int new_size , const T& v ) { if ( new_size > Capacity ) reserve( _grow_capacity( new_size ) ); if ( new_size > Size ) for ( int n = Size; n < new_size; n++ ) memcpy( &Data[ n ] , &v , sizeof( v ) ); Size = new_size; }
	inline void         shrink( int new_size ) { IM_ASSERT( new_size <= Size ); Size = new_size; } // Resize a vec3_t to a smaller size, guaranteed not to cause a reallocation
	inline void         reserve( int new_capacity ) { if ( new_capacity <= Capacity ) return; T* new_data = ( T* ) IM_ALLOC( ( size_t ) new_capacity * sizeof( T ) ); if ( Data ) { memcpy( new_data , Data , ( size_t ) Size * sizeof( T ) ); IM_FREE( Data ); } Data = new_data; Capacity = new_capacity; }

	// NB: It is illegal to call push_back/push_front/insert with a reference pointing inside the Imvec3_t data itself! e.g. v.push_back(v[10]) is forbidden.
...
struct ImGuiIO
{
	...
	ImFont* ButtonFont;                    // = NULL           // Font to use on NewFrame(). Use NULL to uses Fonts->Fonts[0].
	ImFont* CustomFont;                    // = NULL           // Font to use on NewFrame(). Use NULL to uses Fonts->Fonts[0].
	...
	Imvec3_t<ImWchar> InputQueueCharacters;     // Queue of _characters_ input (obtained by platform backend). Fill using AddInputCharacter() helper.
...
struct ImGuiTextFilter
{
	...
	struct ImGuiTextRange
	{
		...
		IMGUI_API void  split( char separator , Imvec3_t<ImGuiTextRange>* out ) const;
	};
	char                    InputBuf[ 256 ];
	Imvec3_t<ImGuiTextRange>Filters;
	int                     CountGrep;
};
...
struct ImGuiStorage
{
	...
	Imvec3_t<ImGuiStoragePair>      Data;
...
{
	Imvec3_t<ImDrawCmd>         _CmdBuffer;
	Imvec3_t<ImDrawIdx>         _IdxBuffer;
};
...
struct ImDrawListSplitter
{
	...
	Imvec3_t<ImDrawChannel>     _Channels;   // Draw channels (not resized down so _Count might be < Channels.Size)
...
struct ImDrawList
{
	...
	Imvec3_t<ImDrawCmd>     CmdBuffer;          // Draw commands. Typically 1 command = 1 GPU draw call, unless the command is a callback.
	Imvec3_t<ImDrawIdx>     IdxBuffer;          // Index buffer. Each command consume ImDrawCmd::ElemCount of those
	Imvec3_t<ImDrawVert>    VtxBuffer;          // Vertex buffer.
	...
	ImDrawVert* _VtxWritePtr;       // [Internal] point within VtxBuffer.Data after each add command (to avoid using the Imvec3_t<> operators too much)
	ImDrawIdx* _IdxWritePtr;       // [Internal] point within IdxBuffer.Data after each add command (to avoid using the Imvec3_t<> operators too much)
	Imvec3_t<ImVec4>        _ClipRectStack;     // [Internal]
	Imvec3_t<ImTextureID>   _TextureIdStack;    // [Internal]
	Imvec3_t<ImVec2>        _Path;              // [Internal] current path building
...
// This is essentially a tightly packed of vec3_t of 64k booleans = 8KB storage.
struct ImFontGlyphRangesBuilder
{
	Imvec3_t<ImU32> UsedChars;            // Store 1-bit per Unicode code point (0=unused, 1=used)
	...
	IMGUI_API void  BuildRanges( Imvec3_t<ImWchar>* out_ranges );                 // Output new ranges
};
...
struct ImFontAtlas
{
	...
	Imvec3_t<ImFont*>           Fonts;              // Hold all the fonts returned by AddFont*. Fonts[0] is the default font upon calling ImGui::NewFrame(), use ImGui::PushFont()/PopFont() to change the current font.
	Imvec3_t<ImFontAtlasCustomRect> CustomRects;    // Rectangles for packing custom texture data into the atlas.
	Imvec3_t<ImFontConfig>      ConfigData;         // Configuration data
...
struct ImFont
{
	...
	Imvec3_t<float>             IndexAdvanceX;      // 12-16 // out //            // Sparse. Glyphs->AdvanceX in a directly indexable way (cache-friendly for CalcTextSize functions which only this this info, and are often bottleneck in large UI).
	float                       FallbackAdvanceX;   // 4     // out // = FallbackGlyph->AdvanceX
	float                       FontSize;           // 4     // in  //            // Height of characters/line, set during loading (don't change after loading)

	// Members: Hot ~28/40 bytes (for CalcTextSize + render loop)
	Imvec3_t<ImWchar>           IndexLookup;        // 12-16 // out //            // Sparse. Index glyphs by Unicode code-point.
	Imvec3_t<ImFontGlyph>       Glyphs;             // 12-16 // out //            // All glyphs.
